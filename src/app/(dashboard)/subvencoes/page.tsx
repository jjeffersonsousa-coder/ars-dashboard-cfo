...
import { Plus, Search, Users, ChevronDown, ChevronRight, CheckSquare, Square, X } from "lucide-react"
...
export default function SubvencoesPage() {
  ...
  const PERIODOS = ANOS.flatMap(a => MESES.map(m => `${m}/${a}`))

  function togglePeriodo(p: string) {
    setSelectedPeriodos(prev => prev.includes(p) ? prev.filter(x => x !== p) : [...prev, p])
  }
  ...
  const filteredRecs = mockSubvencoes.filter(d => {
    ...
    if (selectedPeriodos.length > 0) {
      const dataMes = new Date(d.data + "T00:00:00")
      const mesAno = `${MESES[dataMes.getMonth()]}/${dataMes.getFullYear()}`
      if (!selectedPeriodos.includes(mesAno)) return false
    }
    return true
  })
      ...
      <Card>
        <CardContent className="pt-4 pb-3 space-y-3">
          ...
          <div className="flex flex-col gap-1">
            <label className="text-xs font-semibold text-slate-400 uppercase tracking-wide">Período (mês/ano)</label>
            <div className="flex flex-wrap gap-1.5 items-center">
              {selectedPeriodos.map(p => (
                <span key={p} className="flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium text-white"
                  style={{ backgroundColor: "#006494" }}>
                  {p}
                  <button onClick={() => togglePeriodo(p)}><X className="w-3 h-3 opacity-70 hover:opacity-100" /></button>
                </span>
              ))}
              <div className="relative" ref={periodoRef}>
                <button onClick={() => setShowPeriodoDropdown(!showPeriodoDropdown)}
                  ...
                  {selectedPeriodos.length === 0 ? "Todos os períodos" : "Adicionar período"}
                  <ChevronDown className={`w-3.5 h-3.5 text-slate-400 transition-transform ${showPeriodoDropdown ? "rotate-180" : ""}`} />
                </button>
                {showPeriodoDropdown && (
                  <div className="absolute z-30 top-full mt-1 left-0 bg-white border border-slate-200 rounded-xl shadow-xl" style={{ width: 220, maxHeight: 280, overflowY: "auto" }}>
                    {selectedPeriodos.length > 0 && (
                      <button onClick={() => setSelectedPeriodos([])}
                        className="w-full px-3 py-1.5 text-xs text-red-500 hover:bg-red-50 font-medium border-b border-slate-100 text-left">
                        Limpar ({selectedPeriodos.length})
                      </button>
                    )}
                    {PERIODOS.map(p => (
                      <button key={p} onClick={() => togglePeriodo(p)}
                        className={`w-full text-left px-3 py-2 text-sm transition-colors flex items-center gap-2 ${selectedPeriodos.includes(p) ? "bg-blue-50 font-medium" : "hover:bg-slate-50"}`}>
                        <span className={`w-4 h-4 rounded border shrink-0 flex items-center justify-center ${selectedPeriodos.includes(p) ? "border-transparent" : "border-slate-300"}`}
                          style={selectedPeriodos.includes(p) ? { backgroundColor: "#006494" } : {}}>
                          {selectedPeriodos.includes(p) && <span className="text-white text-xs font-bold leading-none">✓</span>}
                        </span>
                        {p}
                      </button>
                    ))}
                ...
                {showPeriodoDropdown && <div className="fixed inset-0 z-20" onClick={() => setShowPeriodoDropdown(false)} />}
              </div>
            </div>
          </div>
